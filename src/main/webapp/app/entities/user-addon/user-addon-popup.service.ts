            ...
            if (id) {
                this.userAddonService.find(id)
                    .subscribe((userAddonResponse: HttpResponse<UserAddon>) => {
                        const userAddon: UserAddon = userAddonResponse.body;
                        this.ngbModalRef = this.userAddonModalRef(component, userAddon);
                        resolve(this.ngbModalRef);
                    });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.userAddonModalRef(component, new UserAddon());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
